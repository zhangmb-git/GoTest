...
// 加载数据类知识问题
func LoadQuestionsWithData() *list.List {

	xlsPath := TOPIC["xlsPath"]
	if xlsPath == "" {
		panic("xlspath empty")
		return nil
	}
	...
	for _, sheet := range xlFile.Sheets { // 循环sheet
		...
		err := sheet.ForEachRow(func(row *xlsx.Row) error {
			//logger.Sugar.Infof("共 %d 行==", count)
			question := row.GetCell(11)
			role := row.GetCell(15)
			answer := row.GetCell(16)
			enableFlag := row.GetCell(10) // 禁用标准
			// 禁用类的知识不验证
			...
			//InsertAskQuestions(123456, question.Value, formatValue)
			return nil
		})
...
//	lenCol := len(rows[0])
//	if  lenCol<= 17 {
//		logger.Sugar.Errorf("共 %d 列", lenCol)
//		return nil
//	}
//
//	for _, row := range rows {
//
//		question := row.Cell[11]
//		role := row[15]
//		answer := row[16]
//		enableFlag := row[10] // 禁用标准
//
//		// 禁用类的知识不验证
//		if enableFlag.Value == "禁用" {
//			continue
